...
void siren(int repeat)
{
	...
	for (int i=0; i < repeat; i++)
	{
		for (int j=0; j < 23; j++)
		{
			frequency += 35;  // 35 Hz로 add
			set_buzzer(frequency);
			HAL_Delay(20);
		}
		for (int j=0; j < 54; j++)
		{
			frequency -= 15;  // 15 Hz로 sub
			set_buzzer(frequency);
			HAL_Delay(20);
		}
	    noTone();
...
void buzzer_main()
{
  ...
  while (1)
  {
    ...
    for (int i=0; i < 24; i++)
    {
		...
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, divide_freq / school_bell[i] / 2);  // Duty를 50%로 설정 한다.
		HAL_Delay(500);
    ...
    for (int i=0; i < 25; i++)
    {
		...
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, divide_freq / happy_birthday[i] / 2);
